...
2. Find half-clone-conversation.sh: first check `"$PROJECT_DIR/scripts/half-clone-conversation.sh"` where PROJECT_DIR is the project path from step 1. If not found, run `find ~/.claude -name "half-clone-conversation.sh" 2>/dev/null | sort -V | tail -1`
