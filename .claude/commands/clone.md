---
name: clone
description: Clone the current conversation so you can branch off and try a different approach.
---
...
2. Find clone-conversation.sh: first check `"$PROJECT_DIR/scripts/clone-conversation.sh"` where PROJECT_DIR is the project path from step 1. If not found, run `find ~/.claude -name "clone-conversation.sh" 2>/dev/null | sort -V | tail -1`
